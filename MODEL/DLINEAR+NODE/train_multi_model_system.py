...
class SentimentDLinearNodeModel(nn.Module):
    ...
    def forward(self, price_x, sentiment_x):
        ...
        combined_input = torch.cat([price_x, sentiment_x], dim=-1)  # [batch, seq, price_dim + sentiment_dim]
...
class HybridDLinearNodeModel(nn.Module):
    ...
    def forward(self, price_x, residual_x):
        ...
        combined_input = torch.cat([price_x, residual_x], dim=-1)  # [batch, seq, price_dim + residual_dim]
...
class MultiModelDataProcessor:
    ...
    def create_sentiment_features(self, df):
        ...
        for col in sentiment_cols:
            if col in df.columns:
                sentiment_features[col] = pd.to_numeric(df[col], errors='coerce')
        ...
        if 'sentiment_score' in sentiment_features.columns:
            sentiment_features['sentiment_momentum'] = sentiment_features['sentiment_score'].rolling(5).mean()
            sentiment_features['sentiment_volatility'] = sentiment_features['sentiment_score'].rolling(10).std()
        ...
        for col in sentiment_features.columns:
            if sentiment_features[col].std() > 0:
                sentiment_features[col] = (sentiment_features[col] - sentiment_features[col].mean()) / sentiment_features[col].std()
            sentiment_features[col] = np.clip(sentiment_features[col], -3, 3) / 3  # Scale to [-1, 1]
    ...
    def prepare_basic_sequences(self, features_df, prices, stock_name, seq_len=60, scaler_key_override=None):
        ...
        if scaler_key not in self.price_scalers:
            ...
            features_scaled = self.price_scalers[scaler_key].fit_transform(features_df)
        else:
            features_scaled = self.price_scalers[scaler_key].transform(features_df)
        
        # Scale prices
        if scaler_key not in self.feature_scalers:
            self.feature_scalers[scaler_key] = StandardScaler()
            prices_scaled = self.feature_scalers[scaler_key].fit_transform(prices.reshape(-1, 1)).flatten()
        else:
            prices_scaled = self.feature_scalers[scaler_key].transform(prices.reshape(-1, 1)).flatten()
    ...
    def prepare_sentiment_sequences(self, price_features, sentiment_features, prices, stock_name, seq_len=60):
        ...
        if price_key not in self.price_scalers:
            ...
            price_scaled = self.price_scalers[price_key].fit_transform(price_features)
        else:
            price_scaled = self.price_scalers[price_key].transform(price_features)
        ...
        if sentiment_key not in self.sentiment_scalers:
            ...
            sentiment_scaled = self.sentiment_scalers[sentiment_key].fit_transform(sentiment_features)
        else:
            sentiment_scaled = self.sentiment_scalers[sentiment_key].transform(sentiment_features)
        ...
        if target_key not in self.feature_scalers:
            ...
            prices_scaled = self.feature_scalers[target_key].transform(prices.reshape(-1, 1)).flatten()
    ...
    def prepare_hybrid_sequences(self, features_df, prices, stock_name, seq_len=60):
        ...
        if scaler_key not in self.feature_scalers:
            self.feature_scalers[scaler_key] = StandardScaler()
            features_scaled = self.feature_scalers[scaler_key].fit_transform(features_df)
        else:
            features_scaled = self.feature_scalers[scaler_key].transform(features_df)
        ...
        if target_key not in self.price_scalers:
            ...
            prices_scaled = self.price_scalers[target_key].fit_transform(prices.reshape(-1, 1)).flatten()
        else:
            prices_scaled = self.price_scalers[target_key].transform(prices.reshape(-1, 1)).flatten()
...
class MultiModelTrainer:
    ...
    def train_single_model(self, model, train_loader, val_loader, model_name, stock_name, epochs=100):
        ...
        for epoch in range(epochs):
            ...
            if epoch % 20 == 0 or val_r2 > best_r2:
                print(f"  Epoch {epoch:3d}: Loss={avg_train_loss:.6f}, Val_R²={val_r2:.4f} - {status}")
    ...
    def train_all_models_for_stock(self, stock_name, data_paths, epochs=100):
        ...
        X_basic, y_basic = self.processor.prepare_basic_sequences(basic_features, prices, stock_name)
        ...
        if len(X_basic) > 0:
            ...
            basic_model = BasicLSTMModel(input_dim=X_basic.shape[2]).to(self.device)
        ...
        if len(X_price) > 0:
            ...
            X_price_train, X_price_val = X_price[:train_size], X_price[train_size:]
            X_sent_train, X_sent_val = X_sentiment[:train_size], X_sentiment[train_size:]
            y_sent_train, y_sent_val = y_sentiment[:train_size], y_sentiment[train_size:]
            ...
            results['sentiment'] = {'r2': sentiment_r2, 'history': sentiment_history}
        ...
        X_price_hybrid, y_price_hybrid = self.processor.prepare_basic_sequences(price_features, prices, stock_name)
        ...
        X_residual, y_hybrid = self.processor.prepare_basic_sequences(residual_features, prices, stock_name, scaler_key_override=residual_scaler_key)
        ...
        if len(X_price_hybrid) > 0 and len(X_residual) > 0:
            ...
            X_price_train, X_price_val = X_price_hybrid[:train_size], X_price_hybrid[train_size:]
            X_resid_train, X_resid_val = X_residual[:train_size], X_residual[train_size:]
